...
export async function imageConvertResizeReduce(
    ...
    try {
        ...
        if (input.maintainAspectRatio !== false) {
            ...
            // Use lockDimension to determine which value to auto-calculate
            if (input.lockDimension === 'width' && input.width) {
                // User is changing width → auto-calculate height
                targetWidth = input.width
                targetHeight = Math.round(input.width / aspect)
            } else if (input.lockDimension === 'height' && input.height) {
                // User is changing height → auto-calculate width
                ...
                // Both provided but no lock → fit to bounds (original behavior)
        ...
        // Apply fit logic (skipping fill, as its already handled by default)
